...
; 0000 003C // Func5=In Func4=In Func3=In Func2=In Func1=In Func0=Out
; 0000 003D // State5=T State4=T State3=T State2=T State1=T State0=1
...
; 0000 0050 GIMSK=0x00;
	OUT  0x3B,R30
; 0000 0051 MCUCR=0x00;
	OUT  0x35,R30
; 0000 0052 
; 0000 0053 // Timer/Counter 0 Interrupt(s) initialization
; 0000 0054 TIMSK0=0x00;
	OUT  0x39,R30
; 0000 0055 
; 0000 0056 // Analog Comparator initialization
; 0000 0057 // Analog Comparator: Off
; 0000 0058 ACSR=0x80;
	LDI  R30,LOW(128)
	OUT  0x8,R30
; 0000 0059 ADCSRB=0x00;
	LDI  R30,LOW(0)
	OUT  0x3,R30
; 0000 005A 
; 0000 005B // ADC initialization
; 0000 005C // ADC Clock frequency: 300,000 kHz
; 0000 005D // ADC Bandgap Voltage Reference: Off
; 0000 005E // ADC Auto Trigger Source: None
; 0000 005F // Only the 8 most significant bits of
; 0000 0060 // the AD conversion result are used
; 0000 0061 // Digital input buffers on ADC0: Off, ADC1: Off, ADC2: On, ADC3: Off
; 0000 0062 DIDR0&=0x03;
	IN   R30,0x14
	ANDI R30,LOW(0x3)
	OUT  0x14,R30
; 0000 0063 DIDR0|=0x2C;
	IN   R30,0x14
	ORI  R30,LOW(0x2C)
	OUT  0x14,R30
; 0000 0064 ADMUX=ADC_VREF_TYPE & 0xff;
	LDI  R30,LOW(32)
	OUT  0x7,R30
; 0000 0065 ADCSRA=0x84;
	LDI  R30,LOW(132)
	OUT  0x6,R30
; 0000 0066 
; 0000 0067 while (1)
_0x6:
; 0000 0068       {
; 0000 0069       PORTB=0x01;
	LDI  R30,LOW(1)
	OUT  0x18,R30
; 0000 006A       for (i=0; i<480; i++)    delay_ms(60000);   //480=8hours
	RCALL SUBOPT_0x0
_0xA:
	RCALL SUBOPT_0x1
	__CPD2N 0x1E0
	...
	RJMP _0xA
_0xB:
; 0000 006B time = read_adc(2);
	LDI  R30,LOW(2)
	ST   -Y,R30
	RCALL _read_adc
	MOV  R4,R30
; 0000 006C       for (i=0; i<time; i++)   delay_ms(60000);   //60.000
	RCALL SUBOPT_0x0
_0xD:
	MOV  R30,R4
	RCALL SUBOPT_0x1
...
; 0000 006D PORTB=0x00;
	LDI  R30,LOW(0)
	OUT  0x18,R30
; 0000 006E       for (i=time; i<960; i++) delay_ms(60000);   //960=16hours
	MOV  R30,R4
	CLR  R31
	CLR  R22
	CLR  R23
